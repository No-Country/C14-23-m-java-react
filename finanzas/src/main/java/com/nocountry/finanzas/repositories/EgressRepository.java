...
public interface EgressRepository extends JpaRepository<Egress, Long> {
    ...
    @Query("SELECT e FROM Egress  e WHERE e.user.id = :id AND e.egressCategory.id = :egressCategory")
    ...
    @Query("SELECT e.user.id FROM Egress e WHERE e.id = :egressId")
    Long findUserIdByEgressId(@Param("egressId") Long egressId);
